...
final class VandarClient
{
    ...
    {
        return $this->request('GET', $base, $path, ['query' => $query], $auth);
    }
    ...
    {
        return $this->request('POST', $base, $path, ['json' => $payload], $auth);
    }
    ...
    {
        return $this->request('PUT', $base, $path, ['json' => $payload], $auth);
    }
    ...
    {
        return $this->request('PATCH', $base, $path, ['json' => $payload], $auth);
    }
    ...
    {
        return $this->request('DELETE', $base, $path, ['json' => $payload], $auth);
    }

    /**
     * @param  array<string, mixed>  $options
     */
    public function request(string $method, string $base, string $path, array $options = [], bool $auth = true): VandarResponse
    {
        return $this->pendingRequest->send($method, $base, $path, $options, $auth);
    }
}
