...
class VandarException extends RuntimeException
{
    ...
    private static function isSensitiveMessageKey(string $key, string $value): bool
    {
        ...
        return preg_match('/(access[_-]?token|refresh[_-]?token|refreshtoken|authorization|bearer|api[_-]?key|card[_-]?number|iban|national[_-]?code|mobile|phone)/i', $value) === 1;
    }
}
